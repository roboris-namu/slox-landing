...
const COUNTRY_OPTIONS = [
  ...
  { code: "BR", flag: "🇧🇷", name: "ブラジル" },
  { code: "GB", flag: "🇬🇧", name: "イギリス" },
  { code: "CA", flag: "🇨🇦", name: "カナダ" },
  { code: "AU", flag: "🇦🇺", name: "オーストラリア" },
  { code: "IN", flag: "🇮🇳", name: "インド" },
  { code: "RU", flag: "🇷🇺", name: "ロシア" },
  { code: "IT", flag: "🇮🇹", name: "イタリア" },
  { code: "MX", flag: "🇲🇽", name: "メキシコ" },
  { code: "TH", flag: "🇹🇭", name: "タイ" },
  { code: "VN", flag: "🇻🇳", name: "ベトナム" },
  { code: "ID", flag: "🇮🇩", name: "インドネシア" },
  { code: "PH", flag: "🇵🇭", name: "フィリピン" },
  { code: "MY", flag: "🇲🇾", name: "マレーシア" },
  { code: "SG", flag: "🇸🇬", name: "シンガポール" },
  { code: "NZ", flag: "🇳🇿", name: "ニュージーランド" },
...
function LoginContentJA() {
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    } catch (error) { console.error("Avatar upload error:", error); alert("プロフィール写真のアップロードに失敗しました。"); }
  ...
  const handleCheckIn = async () => {
    ...
    try {
      ...
      if (!error) {
        ...
        if (profile) setProfile({ ...profile, total_score: profile.total_score + 10, attendance_count: profile.attendance_count + 1 });
      }
    } finally {
      setCheckingIn(false);
    }
  ...
  const handleEmailSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!emailRegex.test(email)) throw new Error("有効なメールアドレスを入力してください");
      const emailCheckRes = await fetch(`/api/profile?email=${encodeURIComponent(email)}`);
      const emailCheckData = await emailCheckRes.json();
      if (emailCheckData.profile) throw new Error("このメールアドレスは既に登録されています");
      const nicknameCheckRes = await fetch(`/api/profile?nickname=${encodeURIComponent(nickname.trim())}`);
      const nicknameCheckData = await nicknameCheckRes.json();
      if (nicknameCheckData.profile) throw new Error("このニックネームは既に使用されています");
      const { data, error } = await supabase.auth.signUp({ email, password, options: { data: { nickname: nickname.trim() } } });
      if (error) throw error;
      if (data.user) {
        await fetch("/api/profile", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: data.user.id, nickname: nickname.trim(), email, avatar_url: null, total_score: 0, attendance_count: 0 }) });
        alert("会員登録完了！");
        setIsSignUp(false); setEmail(""); setPassword(""); setNickname("");
      }
    } catch (err) { setAuthError(err instanceof Error ? err.message : "登録失敗"); }
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    } catch (err) { setAuthError(err instanceof Error ? err.message : "ログイン失敗"); }
    finally { setAuthLoading(false); }
  };
          ...
          <div className="max-w-md w-full">
            ...
            <div className="bg-dark-800/50 backdrop-blur-xl rounded-2xl border border-white/10 p-6 space-y-4">
              <form onSubmit={isSignUp ? handleEmailSignUp : handleEmailLogin} className="space-y-3">
                {isSignUp && <input type="text" placeholder="ニックネーム (2〜20文字)" value={nickname} onChange={(e) => setNickname(e.target.value)} required minLength={2} maxLength={20} className="w-full px-4 py-3 bg-dark-700 border border-dark-600 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-primary-500" />}
                <input type="email" placeholder="メールアドレス" value={email} onChange={(e) => setEmail(e.target.value)} required className="w-full px-4 py-3 bg-dark-700 border border-dark-600 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-primary-500" />
                <input type="password" placeholder="パスワード (6文字以上)" value={password} onChange={(e) => setPassword(e.target.value)} required minLength={6} className="w-full px-4 py-3 bg-dark-700 border border-dark-600 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-primary-500" />
                {authError && <p className="text-red-400 text-sm text-center">{authError}</p>}
                <button type="submit" disabled={authLoading} className="w-full py-3 bg-primary-600 hover:bg-primary-700 text-white font-semibold rounded-xl transition-colors disabled:opacity-50">{authLoading ? "処理中..." : isSignUp ? "会員登録" : "ログイン"}</button>
              </form>
              <button type="button" onClick={() => { setIsSignUp(!isSignUp); setAuthError(""); }} className="w-full text-center text-gray-400 hover:text-white text-sm transition-colors">{isSignUp ? "アカウントをお持ちですか？ログイン" : "アカウントがありませんか？会員登録"}</button>
            </div>
    ...
    return (
      <main className="min-h-screen bg-dark-950 flex items-center justify-center px-4">
        <div className="max-w-md w-full bg-dark-800/50 rounded-2xl border border-white/10 p-8">
          <h2 className="text-2xl font-bold text-white mb-4 text-center">🎉 ようこそ!</h2>
          <p className="text-dark-400 text-center mb-6">ニックネームを設定してください</p>
          <input type="text" value={setupNickname} onChange={(e) => setSetupNickname(e.target.value)} placeholder={t.profile.nickname} className="w-full px-4 py-3 bg-dark-700 border border-dark-600 rounded-xl text-white mb-4" maxLength={20} />
          <button onClick={handleSetupNickname} disabled={setupNickname.trim().length < 2} className="w-full py-3 bg-gradient-to-r from-indigo-500 to-cyan-500 text-white font-bold rounded-xl disabled:opacity-50">スタート</button>
        </div>
      </main>
    );
  }
  ...
  return (
    <main className="min-h-screen bg-dark-950 relative overflow-hidden">
      <div className="premium-bg" />{showConfetti && <div className="confetti" />}
      <nav className="fixed top-0 left-0 right-0 z-50 p-4"><div className="max-w-6xl mx-auto flex justify-between"><Link href={`/${locale}`} className="flex items-center gap-3"><div className="w-9 h-9 rounded-xl bg-gradient-to-br from-indigo-500 to-cyan-500 flex items-center justify-center"><span className="text-white font-bold text-sm">S</span></div><span className="font-bold text-lg text-white">SLOX</span></Link></div></nav>
        ...
        <div className="max-w-md w-full space-y-6">
          <div className="bg-dark-800/50 backdrop-blur-xl rounded-2xl border border-white/10 p-6">
            <div className="flex items-center gap-4 mb-6">
              <label className="relative cursor-pointer group"><input type="file" accept="image/*" onChange={handleAvatarUpload} disabled={avatarUploading} className="hidden" /><div className="w-16 h-16 rounded-full overflow-hidden ring-2 ring-accent-500 group-hover:opacity-70 transition-opacity">{profile?.avatar_url ? <img src={profile.avatar_url} alt={profile.nickname} className="w-full h-full object-cover" /> : <div className="w-full h-full bg-gradient-to-br from-accent-500 to-cyan-500 flex items-center justify-center text-white text-2xl font-bold">{profile?.nickname?.charAt(0).toUpperCase()}</div>}</div><div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"><span className="text-white text-xl drop-shadow-lg">📷</span></div>{avatarUploading && <div className="absolute inset-0 flex items-center justify-center bg-black/50 rounded-full"><div className="w-6 h-6 border-2 border-white border-t-transparent rounded-full animate-spin"></div></div>}</label>
              <div>{isEditingNickname ? <div className="flex items-center gap-2"><input type="text" value={newNickname} onChange={(e) => setNewNickname(e.target.value)} className="px-3 py-1 bg-dark-700 border border-dark-600 rounded-lg text-white text-sm" maxLength={20} /><button onClick={handleSaveNickname} disabled={nicknameSaving} className="px-3 py-1 bg-accent-500 text-white rounded-lg text-sm">{nicknameSaving ? "..." : t.profile.save}</button><button onClick={() => setIsEditingNickname(false)} className="px-3 py-1 bg-dark-700 text-white rounded-lg text-sm">✕</button></div> : <div className="flex items-center gap-2"><h2 className="text-xl font-bold text-white">{profile?.nickname}</h2><button onClick={() => { setNewNickname(profile?.nickname || ""); setIsEditingNickname(true); }} className="text-dark-500 hover:text-white text-sm">✏️</button></div>}{nicknameError && <p className="text-red-400 text-xs mt-1">{nicknameError}</p>}<p className="text-dark-400 text-sm">{profile?.email}</p><div className="flex items-center gap-2 mt-2">{isEditingCountry ? <div className="flex items-center gap-2"><select value={newCountry} onChange={(e) => setNewCountry(e.target.value)} className="px-2 py-1 bg-dark-700 border border-dark-600 rounded text-white text-sm">{COUNTRY_OPTIONS.map((c) => <option key={c.code} value={c.code}>{c.flag} {c.name}</option>)}</select><button onClick={() => handleCountryChange(newCountry)} disabled={countrySaving} className="px-2 py-1 bg-accent-500 text-white text-xs rounded">{countrySaving ? "..." : "保存"}</button><button onClick={() => setIsEditingCountry(false)} className="px-2 py-1 bg-dark-600 text-gray-300 text-xs rounded">✕</button></div> : <button onClick={() => { setNewCountry(profile?.country || "JP"); setIsEditingCountry(true); }} className="flex items-center gap-1.5 px-2 py-1 bg-dark-700 hover:bg-dark-600 rounded text-sm"><span className="text-lg">{COUNTRY_OPTIONS.find(c => c.code === (profile?.country || "JP"))?.flag || "🌍"}</span><span className="text-dark-400">{COUNTRY_OPTIONS.find(c => c.code === (profile?.country || "JP"))?.name || "その他"}</span><span className="text-gray-500 text-xs">✏️</span></button>}</div></div>
            </div>
