...
const COUNTRY_OPTIONS = [
  ...
  { code: "OTHER", flag: "🌍", name: "其他" },
];

const locale = "zh";
const t = authTranslations[locale];

function LoginContentZH() {
  ...
  const [newCountry, setNewCountry] = useState("CN");
  ...
  const handlePasswordChange = async () => {
    if (!user) return;
    setPasswordError("");
    if (newPassword.length < 6) { setPasswordError("密码至少需要6个字符"); return; }
    if (newPassword !== confirmPassword) { setPasswordError("密码不匹配"); return; }
    ...
    } catch (err) { setPasswordError(err instanceof Error ? err.message : "密码更改失败"); }
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      alert("头像已更新！");
    } catch (error) { console.error("Avatar upload error:", error); alert("头像上传失败。"); }
  ...
  const handleEmailSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!emailRegex.test(email)) throw new Error("请输入有效的电子邮件地址");
      const emailCheckRes = await fetch(`/api/profile?email=${encodeURIComponent(email)}`);
      if ((await emailCheckRes.json()).profile) throw new Error("该邮箱已被注册");
      const nicknameCheckRes = await fetch(`/api/profile?nickname=${encodeURIComponent(nickname.trim())}`);
      if ((await nicknameCheckRes.json()).profile) throw new Error("该昵称已被使用");
      const { data, error } = await supabase.auth.signUp({ email, password, options: { data: { nickname: nickname.trim() } } });
      if (error) throw error;
      if (data.user) { await fetch("/api/profile", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: data.user.id, nickname: nickname.trim(), email, avatar_url: null, total_score: 0, attendance_count: 0 }) }); alert("注册成功！"); setIsSignUp(false); setEmail(""); setPassword(""); setNickname(""); }
    } catch (err) { setAuthError(err instanceof Error ? err.message : "注册失败"); } finally { setAuthLoading(false); }
  };
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    } catch (err) { setAuthError(err instanceof Error ? err.message : "登录失败"); } finally { setAuthLoading(false); }
  };
          ...
          <div className="max-w-md w-full">
            ...
            <div className="bg-dark-800/50 backdrop-blur-xl rounded-2xl border border-white/10 p-6 space-y-4">
              <form onSubmit={isSignUp ? handleEmailSignUp : handleEmailLogin} className="space-y-3">
                {isSignUp && <input type="text" placeholder="昵称 (2-20字符)" value={nickname} onChange={(e) => setNickname(e.target.value)} required minLength={2} maxLength={20} className="w-full px-4 py-3 bg-dark-700 border border-dark-600 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-primary-500" />}
                <input type="email" placeholder="电子邮件" value={email} onChange={(e) => setEmail(e.target.value)} required className="w-full px-4 py-3 bg-dark-700 border border-dark-600 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-primary-500" />
                <input type="password" placeholder="密码 (6位以上)" value={password} onChange={(e) => setPassword(e.target.value)} required minLength={6} className="w-full px-4 py-3 bg-dark-700 border border-dark-600 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-primary-500" />
                {authError && <p className="text-red-400 text-sm text-center">{authError}</p>}
                <button type="submit" disabled={authLoading} className="w-full py-3 bg-primary-600 hover:bg-primary-700 text-white font-semibold rounded-xl transition-colors disabled:opacity-50">{authLoading ? "处理中..." : isSignUp ? "注册" : "登录"}</button>
              </form>
              <button type="button" onClick={() => { setIsSignUp(!isSignUp); setAuthError(""); }} className="w-full text-center text-gray-400 hover:text-white text-sm transition-colors">{isSignUp ? "已有账户？登录" : "没有账户？注册"}</button>
            </div>
          ...
          <div className="bg-dark-700/50 rounded-xl p-4">
            {isEditingPassword ? (
              <div className="space-y-3">
                <h3 className="text-white font-semibold text-center mb-3">🔐 修改密码</h3>
                <input type="password" placeholder="新密码 (至少6个字符)" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} className="w-full px-3 py-2 bg-dark-600 border border-dark-500 rounded-lg text-white text-sm" />
                <input type="password" placeholder="确认新密码" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="w-full px-3 py-2 bg-dark-600 border border-dark-500 rounded-lg text-white text-sm" />
                {passwordError && <p className="text-red-400 text-xs text-center">{passwordError}</p>}
                <div className="flex gap-2">
                  <button onClick={handlePasswordChange} disabled={passwordSaving || !newPassword || !confirmPassword} className="flex-1 py-2 bg-primary-600 hover:bg-primary-700 text-white text-sm font-semibold rounded-lg disabled:opacity-50">{passwordSaving ? "修改中..." : "修改"}</button>
                  <button onClick={() => { setIsEditingPassword(false); setNewPassword(""); setConfirmPassword(""); setPasswordError(""); }} className="flex-1 py-2 bg-dark-600 hover:bg-dark-500 text-gray-300 text-sm rounded-lg">取消</button>
                </div>
              </div>
            ) : (
              <button onClick={() => setIsEditingPassword(true)} className="w-full text-center text-gray-400 hover:text-white text-sm transition-colors">🔐 修改密码</button>
            )}
          </div>
