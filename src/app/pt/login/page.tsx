...
const COUNTRY_OPTIONS = [
  ...
  { code: "NZ", flag: "🇳🇿", name: "Nova Zelândia" },
...
function LoginContentPT() {
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file || !user) return;
    if (file.size > 5 * 1024 * 1024) { alert("O tamanho do arquivo deve ser menor que 5MB."); return; }
    ...
    } catch (error) { console.error("Avatar upload error:", error); alert("Falha ao enviar foto de perfil."); }
  ...
  const handleEmailSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!emailRegex.test(email)) throw new Error("Por favor insira um email válido");
      const emailCheckRes = await fetch(`/api/profile?email=${encodeURIComponent(email)}`);
      if ((await emailCheckRes.json()).profile) throw new Error("Este email já está registrado");
      const nicknameCheckRes = await fetch(`/api/profile?nickname=${encodeURIComponent(nickname.trim())}`);
      if ((await nicknameCheckRes.json()).profile) throw new Error("Este apelido já está em uso");
      const { data, error } = await supabase.auth.signUp({ email, password, options: { data: { nickname: nickname.trim() } } });
      if (error) throw error;
      if (data.user) { await fetch("/api/profile", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: data.user.id, nickname: nickname.trim(), email, avatar_url: null, total_score: 0, attendance_count: 0 }) }); alert("Registro concluído!"); setIsSignUp(false); setEmail(""); setPassword(""); setNickname(""); }
    } catch (err) { setAuthError(err instanceof Error ? err.message : "Erro no registro"); } finally { setAuthLoading(false); }
  };
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    } catch (err) { setAuthError(err instanceof Error ? err.message : "Erro no login"); } finally { setAuthLoading(false); }
  };
          ...
          <div className="max-w-md w-full">
            ...
            <div className="bg-dark-800/50 backdrop-blur-xl rounded-2xl border border-white/10 p-6 space-y-4">
              <form onSubmit={isSignUp ? handleEmailSignUp : handleEmailLogin} className="space-y-3">
                {isSignUp && <input type="text" placeholder="Apelido (2-20 caracteres)" value={nickname} onChange={(e) => setNickname(e.target.value)} required minLength={2} maxLength={20} className="w-full px-4 py-3 bg-dark-700 border border-dark-600 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-primary-500" />}
                ...
                <input type="password" placeholder="Senha (6+ caracteres)" value={password} onChange={(e) => setPassword(e.target.value)} required minLength={6} className="w-full px-4 py-3 bg-dark-700 border border-dark-600 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-primary-500" />
                {authError && <p className="text-red-400 text-sm text-center">{authError}</p>}
                <button type="submit" disabled={authLoading} className="w-full py-3 bg-primary-600 hover:bg-primary-700 text-white font-semibold rounded-xl transition-colors disabled:opacity-50">{authLoading ? "Processando..." : isSignUp ? "Registrar" : "Entrar"}</button>
              </form>
              <button type="button" onClick={() => { setIsSignUp(!isSignUp); setAuthError(""); }} className="w-full text-center text-gray-400 hover:text-white text-sm transition-colors">{isSignUp ? "Já tem conta? Entrar" : "Não tem conta? Registrar"}</button>
            </div>
