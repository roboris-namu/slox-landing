...
const COUNTRY_OPTIONS = [
  ...
  { code: "BR", flag: "🇧🇷", name: "Brasilien" },
  { code: "GB", flag: "🇬🇧", name: "Großbritannien" },
  { code: "CA", flag: "🇨🇦", name: "Kanada" },
  { code: "AU", flag: "🇦🇺", name: "Australien" },
  { code: "IN", flag: "🇮🇳", name: "Indien" },
  { code: "RU", flag: "🇷🇺", name: "Russland" },
  { code: "IT", flag: "🇮🇹", name: "Italien" },
  { code: "MX", flag: "🇲🇽", name: "Mexiko" },
  { code: "TH", flag: "🇹🇭", name: "Thailand" },
  { code: "VN", flag: "🇻🇳", name: "Vietnam" },
  { code: "ID", flag: "🇮🇩", name: "Indonesien" },
  { code: "PH", flag: "🇵🇭", name: "Philippinen" },
  { code: "MY", flag: "🇲🇾", name: "Malaysia" },
  { code: "SG", flag: "🇸🇬", name: "Singapur" },
  { code: "NZ", flag: "🇳🇿", name: "Neuseeland" },
...
function LoginContentDE() {
  ...
  const handlePasswordChange = async () => {
    if (!user) return;
    setPasswordError("");
    if (newPassword.length < 6) { setPasswordError("Passwort muss mindestens 6 Zeichen haben"); return; }
    ...
    } catch (err) { setPasswordError(err instanceof Error ? err.message : "Passwortänderung fehlgeschlagen"); }
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file || !user) return;
    if (file.size > 5 * 1024 * 1024) { alert("Dateigröße muss unter 5MB sein."); return; }
    ...
    } catch (error) { console.error("Avatar upload error:", error); alert("Profilbild-Upload fehlgeschlagen."); }
  ...
  const handleEmailSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!emailRegex.test(email)) throw new Error("Bitte geben Sie eine gültige E-Mail ein");
      const emailCheckRes = await fetch(`/api/profile?email=${encodeURIComponent(email)}`);
      if ((await emailCheckRes.json()).profile) throw new Error("Diese E-Mail ist bereits registriert");
      const nicknameCheckRes = await fetch(`/api/profile?nickname=${encodeURIComponent(nickname.trim())}`);
      if ((await nicknameCheckRes.json()).profile) throw new Error("Dieser Nickname ist bereits vergeben");
      const { data, error } = await supabase.auth.signUp({ email, password, options: { data: { nickname: nickname.trim() } } });
      if (error) throw error;
      if (data.user) { await fetch("/api/profile", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: data.user.id, nickname: nickname.trim(), email, avatar_url: null, total_score: 0, attendance_count: 0 }) }); alert("Registrierung abgeschlossen!"); setIsSignUp(false); setEmail(""); setPassword(""); setNickname(""); }
    } catch (err) { setAuthError(err instanceof Error ? err.message : "Registrierung fehlgeschlagen"); } finally { setAuthLoading(false); }
  };
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    } catch (err) { setAuthError(err instanceof Error ? err.message : "Anmeldung fehlgeschlagen"); } finally { setAuthLoading(false); }
  };
          ...
          <div className="max-w-md w-full">
            ...
            <div className="bg-dark-800/50 backdrop-blur-xl rounded-2xl border border-white/10 p-6 space-y-4">
              <form onSubmit={isSignUp ? handleEmailSignUp : handleEmailLogin} className="space-y-3">
                {isSignUp && <input type="text" placeholder="Nickname (2-20 Zeichen)" value={nickname} onChange={(e) => setNickname(e.target.value)} required minLength={2} maxLength={20} className="w-full px-4 py-3 bg-dark-700 border border-dark-600 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-primary-500" />}
                <input type="email" placeholder="E-Mail" value={email} onChange={(e) => setEmail(e.target.value)} required className="w-full px-4 py-3 bg-dark-700 border border-dark-600 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-primary-500" />
                <input type="password" placeholder="Passwort (6+ Zeichen)" value={password} onChange={(e) => setPassword(e.target.value)} required minLength={6} className="w-full px-4 py-3 bg-dark-700 border border-dark-600 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-primary-500" />
                {authError && <p className="text-red-400 text-sm text-center">{authError}</p>}
                <button type="submit" disabled={authLoading} className="w-full py-3 bg-primary-600 hover:bg-primary-700 text-white font-semibold rounded-xl transition-colors disabled:opacity-50">{authLoading ? "Verarbeitung..." : isSignUp ? "Registrieren" : "Anmelden"}</button>
              </form>
              <button type="button" onClick={() => { setIsSignUp(!isSignUp); setAuthError(""); }} className="w-full text-center text-gray-400 hover:text-white text-sm transition-colors">{isSignUp ? "Schon ein Konto? Anmelden" : "Kein Konto? Registrieren"}</button>
            </div>
          ...
          <div className="bg-dark-700/50 rounded-xl p-4">
            {isEditingPassword ? (
              <div className="space-y-3">
                <h3 className="text-white font-semibold text-center mb-3">🔐 Passwort Ändern</h3>
                <input type="password" placeholder="Neues Passwort (mind. 6 Zeichen)" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} className="w-full px-3 py-2 bg-dark-600 border border-dark-500 rounded-lg text-white text-sm" />
                <input type="password" placeholder="Neues Passwort bestätigen" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="w-full px-3 py-2 bg-dark-600 border border-dark-500 rounded-lg text-white text-sm" />
                {passwordError && <p className="text-red-400 text-xs text-center">{passwordError}</p>}
                <div className="flex gap-2">
                  <button onClick={handlePasswordChange} disabled={passwordSaving || !newPassword || !confirmPassword} className="flex-1 py-2 bg-primary-600 hover:bg-primary-700 text-white text-sm font-semibold rounded-lg disabled:opacity-50">{passwordSaving ? "Ändern..." : "Ändern"}</button>
                  <button onClick={() => { setIsEditingPassword(false); setNewPassword(""); setConfirmPassword(""); setPasswordError(""); }} className="flex-1 py-2 bg-dark-600 hover:bg-dark-500 text-gray-300 text-sm rounded-lg">Abbrechen</button>
                </div>
              </div>
            ) : (
              <button onClick={() => setIsEditingPassword(true)} className="w-full text-center text-gray-400 hover:text-white text-sm transition-colors">🔐 Passwort Ändern</button>
            )}
          </div>
