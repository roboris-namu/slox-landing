 ...
 *
 * NOTE: 어드민 미리보기는 컨텍스트가 1건뿐이라 [S1] 출처 마커가 항상 자기 자신을
 *       가리켜 정보 가치가 없습니다. 따라서 프롬프트에서 출처 마커를 금지합니다.
 *       (실제 사용자 챗봇은 여러 카드를 컨텍스트로 넣어 [S1][S2] 등이 의미를 가짐)
 */
...
(2) 그 질문에 대한 답변을 작성하세요.
    ...
    - 단계가 있으면 ①②③ 사용
    - 친근한 어조 ("~합니다", "확인해보세요")
    - 출처 마커([S1] 등) 절대 표시하지 마세요. 어떤 형태의 출처 표기도 금지.
...
export async function GET(req: NextRequest, { params }: RouteContext) {
  ...
  // 안전망: 모델이 프롬프트를 무시하고 [S1] 마커를 붙이는 경우를 대비해 제거.
  // 어드민 미리보기에서는 출처가 항상 자기 자신이라 마커가 무의미합니다.
  const rawAnswer =
    typeof parsed.answer === 'string' ? parsed.answer : '';
  const answer = rawAnswer
    .replace(/\s*\[S\d+\]/g, '') // "[S1]" / " [S2]" 등 모두 제거
    .replace(/[ \t]+([.,!?])/g, '$1') // 마커 제거 후 남는 어색한 공백 정리
    .trim();
