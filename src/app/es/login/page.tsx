...
function LoginContentES() {
  ...
  const handlePasswordChange = async () => {
    ...
    } catch (err) { setPasswordError(err instanceof Error ? err.message : "Error al cambiar contraseña"); }
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file || !user) return;
    if (file.size > 5 * 1024 * 1024) { alert("El tamaño del archivo debe ser menor de 5MB."); return; }
    ...
    } catch (error) { console.error("Avatar upload error:", error); alert("Error al subir la foto de perfil."); }
  ...
  const handleEmailSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!emailRegex.test(email)) throw new Error("Por favor ingrese un correo válido");
      const emailCheckRes = await fetch(`/api/profile?email=${encodeURIComponent(email)}`);
      if ((await emailCheckRes.json()).profile) throw new Error("Este correo ya está registrado");
      const nicknameCheckRes = await fetch(`/api/profile?nickname=${encodeURIComponent(nickname.trim())}`);
      if ((await nicknameCheckRes.json()).profile) throw new Error("Este apodo ya está en uso");
      const { data, error } = await supabase.auth.signUp({ email, password, options: { data: { nickname: nickname.trim() } } });
      if (error) throw error;
      if (data.user) { await fetch("/api/profile", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: data.user.id, nickname: nickname.trim(), email, avatar_url: null, total_score: 0, attendance_count: 0 }) }); alert("¡Registro completado!"); setIsSignUp(false); setEmail(""); setPassword(""); setNickname(""); }
    } catch (err) { setAuthError(err instanceof Error ? err.message : "Error de registro"); } finally { setAuthLoading(false); }
  };
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    } catch (err) { setAuthError(err instanceof Error ? err.message : "Error de inicio de sesión"); } finally { setAuthLoading(false); }
  };
          ...
          <div className="max-w-md w-full">
            ...
            <div className="bg-dark-800/50 backdrop-blur-xl rounded-2xl border border-white/10 p-6 space-y-4">
              <form onSubmit={isSignUp ? handleEmailSignUp : handleEmailLogin} className="space-y-3">
                {isSignUp && <input type="text" placeholder="Apodo (2-20 caracteres)" value={nickname} onChange={(e) => setNickname(e.target.value)} required minLength={2} maxLength={20} className="w-full px-4 py-3 bg-dark-700 border border-dark-600 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-primary-500" />}
                <input type="email" placeholder="Correo electrónico" value={email} onChange={(e) => setEmail(e.target.value)} required className="w-full px-4 py-3 bg-dark-700 border border-dark-600 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-primary-500" />
                <input type="password" placeholder="Contraseña (6+ caracteres)" value={password} onChange={(e) => setPassword(e.target.value)} required minLength={6} className="w-full px-4 py-3 bg-dark-700 border border-dark-600 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-primary-500" />
                {authError && <p className="text-red-400 text-sm text-center">{authError}</p>}
                <button type="submit" disabled={authLoading} className="w-full py-3 bg-primary-600 hover:bg-primary-700 text-white font-semibold rounded-xl transition-colors disabled:opacity-50">{authLoading ? "Procesando..." : isSignUp ? "Registrarse" : "Iniciar sesión"}</button>
              </form>
              <button type="button" onClick={() => { setIsSignUp(!isSignUp); setAuthError(""); }} className="w-full text-center text-gray-400 hover:text-white text-sm transition-colors">{isSignUp ? "¿Ya tienes cuenta? Iniciar sesión" : "¿No tienes cuenta? Registrarse"}</button>
            </div>
          ...
          <div className="bg-dark-700/50 rounded-xl p-4">
            {isEditingPassword ? (
              <div className="space-y-3">
                <h3 className="text-white font-semibold text-center mb-3">🔐 Cambiar Contraseña</h3>
                <input type="password" placeholder="Nueva contraseña (mín. 6 caracteres)" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} className="w-full px-3 py-2 bg-dark-600 border border-dark-500 rounded-lg text-white text-sm" />
                <input type="password" placeholder="Confirmar nueva contraseña" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="w-full px-3 py-2 bg-dark-600 border border-dark-500 rounded-lg text-white text-sm" />
                {passwordError && <p className="text-red-400 text-xs text-center">{passwordError}</p>}
                <div className="flex gap-2">
                  <button onClick={handlePasswordChange} disabled={passwordSaving || !newPassword || !confirmPassword} className="flex-1 py-2 bg-primary-600 hover:bg-primary-700 text-white text-sm font-semibold rounded-lg disabled:opacity-50">{passwordSaving ? "Cambiando..." : "Cambiar"}</button>
              ...
              <button onClick={() => setIsEditingPassword(true)} className="w-full text-center text-gray-400 hover:text-white text-sm transition-colors">🔐 Cambiar Contraseña</button>
            )}
          </div>
